      ...
      <PopoverContent align="end" className="w-[360px] p-0">
        ...
        <Tabs defaultValue="all" className="p-3">
          <TabsList className="w-full">
            ...
            <TabsTrigger value="unread" className="flex-1">Unread {unread > 0 && `(${unread})`}</TabsTrigger>
          </TabsList>
          <TabsContent value="all" className="mt-3">
            <NotificationList items={items} onMark={markOne} />
          </TabsContent>
          <TabsContent value="unread" className="mt-3">
            <NotificationList items={items.filter((n) => !n.read)} onMark={markOne} />
          </TabsContent>
        </Tabs>
        <Separator />
...
function NotificationList({
  ...
  if (items.length === 0) {
    return <div className="py-8 text-center text-xs text-muted-foreground">Nothing to show.</div>;
  }
  return (
