...
interface Props {
  ...
  canPersist: boolean;
  onSaved: (saved: BusinessCategory) => void;
  onCancel: () => void;
}

export function BusinessCategoryForm({ category, canPersist, onSaved, onCancel }: Props) {
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
        <div className="grid gap-6 md:grid-cols-[280px_1fr]">
          <aside className="space-y-4 md:border-e md:border-border md:pe-5">
            <div>
              <Label htmlFor="bcat-slug">{t("slug")}</Label>
              <Input
                id="bcat-slug"
                className="mt-1"
                value={form.slug}
            ...
            <div>
              <Label htmlFor="bcat-icon">{t("iconKey")}</Label>
              <Input
                id="bcat-icon"
                className="mt-1"
                value={form.iconKey}
            ...
            <div>
              <Label htmlFor="bcat-sort">{t("sortOrder")}</Label>
              <Input
                id="bcat-sort"
                className="mt-1"
                type="number"
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label htmlFor={`bcat-name-${activeLocale}`}>
                {t("nameLocale", { locale: LOCALE_META[activeLocale].englishName })}
              </Label>
              <Input
                id={`bcat-name-${activeLocale}`}
