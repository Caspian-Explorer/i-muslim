...
import { getLanguages, getLanguageName } from "@/lib/languages";
...
export type LanguageComboboxProps = SingleProps | MultiProps;
...
export function LanguageCombobox(props: LanguageComboboxProps) {
  ...
  const languages = React.useMemo(() => getLanguages(locale), [locale]);
  ...
  const triggerLabel = (() => {
    if (selectedCodes.length === 0) {
      return placeholder ?? (multiple ? t("selectLanguages") : t("selectLanguage"));
    }
    if (!multiple) return getLanguageName(selectedCodes[0]!, locale);
    return null;
  })();
            ...
            <div className="flex flex-wrap items-center gap-1 min-w-0 py-0.5">
              {selectedCodes.slice(0, MAX_VISIBLE_CHIPS).map((code) => (
                ...
                >
                  ...
                  {!disabled && (
                    <span
                      role="button"
                      aria-label={t("removeChip", { name: getLanguageName(code, locale) })}
              ...
              {selectedCodes.length > MAX_VISIBLE_CHIPS && (
                <span className="text-xs text-muted-foreground">
                  {t("moreLanguages", { count: selectedCodes.length - MAX_VISIBLE_CHIPS })}
                </span>
              )}
            </div>
          ) : (
      ...
      <PopoverContent align="start" className="w-72 p-0">
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              {languages.map((l) => {
                const isSelected = selectedCodes.includes(l.code);
                return (
                  <CommandItem
                    key={l.code}
                    value={`${l.code} ${l.name}`}
