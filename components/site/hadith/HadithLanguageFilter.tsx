...
interface HadithLanguageFilterProps {
  availableLangs: readonly LangCode[];
}

export function HadithLanguageFilter({ availableLangs }: HadithLanguageFilterProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const locale = useLocale();
  const t = useTranslations("hadithLanguageFilter");
